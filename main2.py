...
if __name__ == "__main__":
    ...
    for img in range(2):
        output_PSNR = open(f"tests/sample{img+3}PSNR.csv", 'w')
        output_PSNR.write(f"sample{img+3},Noised,Naive,Regolarized,Regolarized 2nd,TV correction\n")
        output_PSNR.close()
        output_MSE = open(f"tests/sample{img+3}MSE.csv", 'w')
        output_MSE.write(f"sample{img+3},Noised,Naive,Regolarized,Regolarized 2nd,TV correction\n")
        output_MSE.close()
        for i in range(3):
            for j in range(5):
                for q in range(5):
                    iteration = f"K{i+1}_{sigma[j]}_{lambda_value[q]}"
                    main(dim_kernel[i], ker_sigma[i], sigma[j], lambda_value[q], iteration, img+3)
